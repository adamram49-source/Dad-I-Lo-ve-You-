...
/* יצירת לוח */
function drawBoard(clickHandler, removed = []) {
...
function startBombPhase(data) {
  ...
  document.getElementById("turnText").innerText =
    myTurn ? "בחר 3 פצצות על הלוח שלך" : "תור היריב לבחור פצצות";
  ...
  drawBoard(async (i, div) => {
    ...
    if (!isMyHalf) return; // רק הלוח הקרוב שלך
    if (chosen.includes(i) || chosen.length >= 3) return;

    chosen.push(i);
    div.remove(); // נעלם במקום צביעה
